...
date: "January 20, 2019 (updated: `r Sys.Date()`)"
output: 
  html_document:
    theme: flatly
    highlight: haddock 
...
boi_raw_text <- readtext("05-text-as-data/docs/*",
...
```{r}
boi_corpus_seg <- boi_corpus %>% 
  corpus_segment(pattern = "THE NARROW-FORUM DISCUSSION",
                 case_insensitive = FALSE,
                 extract_pattern = FALSE) 

boi_courpus_seg_docs <- boi_corpus_seg$documents %>% 
  janitor::clean_names() %>% 
  filter(segid == 2) %>% 
...
df_chng <- read_excel(path = "05-text-as-data/data/rate-changes.xlsx",
                      sheet = "data")
...
## Visualizing LDA using the `text2vec` package
```{r}
